...
print_help()
{
  ...
  -fake
  -keep_tmp
  -matlabBIN <path/to/matlab>  Default is $matlabBIN
...
my_do_cmd dwiextract -fslgrad $bvecs_yNeg $bvals_yNeg -bzero $DWI_yNeg $b0s_yNeg
my_do_cmd dwiextract -fslgrad $bvecs_yPos $bvals_yPos -bzero $DWI_yPos $b0s_yPos
...
if [ -f $acqParams ]; then rm $acqParams;fi
nvols=`fslnvols $b0s_yNeg`
firstNegIndex=1
...
firstPosIndex=$(( $nvols +1 ))
nvols=`fslnvols $b0s_yPos`
for i in `seq 1 $nvols`
do
  echo 0 1 0 $C4topup >> $acqParams
done
