...
Usage() {
    ...
    echo ""
    echo "OPTIONS"
    echo " -f     converts data to 32bit floating point using PAR-REC pixel scaling"
    echo " -no_q  AVOIDS saving rotation matrix in nifti file (qform matrix)
                  Default is to save it."
    echo " -Q     saves matrix saying file is radiological"
    echo " -m     only generates .omat file, no image conversion"
    echo ""
    echo "Downloaded from http://www.staff.ncl.ac.uk/m.j.firbank/nifti_convert.html"
    echo "Slight modifications by Luis Concha, INB, UNAM. lconcha@unam.mx. Feb 2013."
    exit
}
TooMany() {
...
for argv in $*; do
  ...
  if [ $ct -gt 2 ]; then
    ...
    "-f")
      ...
      ;;
    "-no_q") 
      qf=0    #output matrix in nifti qform
      ;;
    "-Q")
