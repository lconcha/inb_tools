...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp('MV_tool: Create_New_Button');
...
if isempty(findobj(hToolbar_Children,'Tag', 'figViewImages')) 
   ...
   219   220   221 ...
	...
	position = 9;
	separator = 'On';
	hMenus = [ hWindowLevelMenu, hPanZoomMenu,hROIToolMenu, hPointToolMenu, hRotateToolMenu ];
	if length(hMenus>0) 
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp'MV_tool: Activate View Images');
...
if ~isempty(hToolbar)
	...
	PZ = findobj('Tag', 'figPanZoom');
	RT = findobj('Tag', 'figROITool');
	MV = findobj('Tag', 'figViewImages');
	PM = findobj('Tag', 'figPointTool');
	RotT = findobj('Tag', 'figRotateTool');
	
	old_ToolHandles  =     [Rot3D, ZoomO, ZoomI,WL,PZ,RT,PM,RotT];
	old_ToolEnables  = get([Rot3D, ZoomO, ZoomI,WL,PZ,RT,PM,RotT], 'Enable');
	old_ToolStates   = get([Rot3D, ZoomO, ZoomI,WL,PZ,RT,PM,RotT], 'State');
...
% open new figure
fig2 = openfig('MV_tool_figure','reuse');
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp'MV_tool: Deactivate View Images');
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%disp'MV_tool: Reset_Frame_Limit');
