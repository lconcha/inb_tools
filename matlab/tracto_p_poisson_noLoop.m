function p = tracto_p_poisson_noLoop(seed_map,mu_map,v_null_map,p_thresh,areFileNames,niftiOut)
...
%     [x, y, z] = meshgrid(1:1:size(seed_map,2),1:1:size(seed_map,1),1:1:size(seed_map,3));
%     [xi,yi,zi] = meshgrid(1:1:size(mu_map,2),  1:1:size(mu_map,1),  1:1:size(mu_map,3));
%     seed_i = interp3(x,y,z,seed_map.*100,xi,yi,zi,'cubic');
%     index_seed = find(seed_i > 0);
%     fprintf(1,'Size of seed_i is %d\n',length(index_seed))
%     p(index_seed) = 0;
    index_seed = find(seed_map > 0);
